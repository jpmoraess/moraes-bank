...
func (q *Queries) GetTransfersFromAccount(ctx context.Context, fromAccountID int64) ([]Transfer, error) {
	rows, err := q.db.Query(ctx, getTransfersFromAccount, fromAccountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetTransfersToAccount(ctx context.Context, toAccountID int64) ([]Transfer, error) {
	rows, err := q.db.Query(ctx, getTransfersToAccount, toAccountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
